...
def rec(pipeline):
    ...
    pred = pipeline.predict(mfcc_feat)
    print('------------------')
    print('Digit : ', pred)
    print('------------------')
...
def collection():
    ...
    for ii in range(0,10):
        ...
        with wave.open('.Digits_Recognition_Project/Rec/Rec_'+str(ii)+'_.wav', mode='wb') as wb:
        ...
        if any(File.endswith(".csv") for File in os.listdir('./Dataset/')):
            ...
            df.to_csv('.Digits_Recognition/DataSet/DataSet__'+name_generator+'__.csv', index=False) 
    return df.head()
